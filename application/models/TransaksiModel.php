...
class TransaksiModel extends CI_Model
{
    ...
    public function getLaporanBarang($tgl1, $tgl2)
    {
       // $this->db->join('user u', 'u.idUser=t.idUser');
        if ($tgl1 != null && $tgl2 != null) {
            $this->db->where('tanggal' . ' >=', $tgl1);
    ...
    public function getTotalBarang($bln = null, $custom = [])
    {
        if ($bln != null) {
            $this->db->like('tanggal', $bln, 'after');
        }
        ...
        $this->db->select_sum('harga', 'totalTransaksi');
        return $this->db->get('barang')->row()->totalTransaksi;
    }
